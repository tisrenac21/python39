...
def addSungJuk(tot, avg, grd):
    ...
    # 총점, 평균, 학점을 계산한 후
    # 그 결과를 전달된 tot, avf, grd에 저장함.
    # addSungJuk 함수에서 계산된 결과 확인 - 실패!
    compluteSungjuk(kor, eng, mat, tot, avg, grd)
...
def compluteSungjuk(kor, eng, mat, tot, avg, grd):
    tot = kor + eng + mat
    avg = tot / 3
    grd = '가'
    if avg >= 90:
...
while True:
    ...
    if menu == '1': addSungJuk(tot, avg, grd)
